...
import CategoryContainer from '@/containers/search/CategoryContainer'
export default {
  components: { CategoryContainer, SearchContainer },
  layout: 'influencer',
  validate ({store}) {
  ...
  async asyncData (context) {
    const { store } = context
    await store.dispatch('search/handleCategories')
    const categories = store.state.search.categories
    return {
      categories
    }
  },
  head () {
    return {
      title: `${this.$t('Каталог товаров ShopShop')}`
    }
  }
}
</script>
